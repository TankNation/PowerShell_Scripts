#############################################################################################################################################
###~ Active Directory - Enable OU Protection ################################################################################################
#
#~ Brian Tancredi
#~ Created: 2017-02-13
#~ Modified: 2017-02-13
...
$target_OU = Get-ADOrganizationalUnit -Identity $named_OU -Properties ProtectedFromAccidentalDeletion #| where {$_.ProtectedFromAccidentalDeletion -eq $false}
...
#############################################################################################################################################
###~ Enable ProtectFromAccidentalDeletion on targeted OU ####################################################################################
#############################################################################################################################################

If ($target_OU.ProtectedFromAccidentalDeletion -eq $false){
    $target_OU | Set-ADObject -ProtectedFromAccidentalDeletion $true
    $fail = 0
}
Else{
    $fail = 1
...
$target_New = Get-ADOrganizationalUnit -Identity $named_OU -Properties ProtectedFromAccidentalDeletion #| where {$_.ProtectedFromAccidentalDeletion -eq $true}
